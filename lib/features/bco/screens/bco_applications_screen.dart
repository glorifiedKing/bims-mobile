...
class _BcoApplicationsScreenState extends State<BcoApplicationsScreen> {
  ...
  void initState() {
    super.initState();
    context.read<BcoApplicationsBloc>().add(FetchBcoApplications(status: 'ALL'));
  }
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: Row(
                            children: const [
                              Icon(Icons.folder, size: 14, color: AppTheme.accentGold),
                              SizedBox(width: 5),
                              Text(
                                'APPLICATIONS',
                                style: TextStyle(
                ...
                builder: (context, state) {
                  ...
                  return Row(
                    children: [
                      ...
                      _buildChip(context, 'IN-REVIEW', activeFilter == 'IN-REVIEW'),
                      _buildChip(context, 'AWAITING ACTION', activeFilter == 'AWAITING ACTION'),
                      _buildChip(context, 'REJECTED', activeFilter == 'REJECTED'),
                    ],
                  );
                },
              ),
                  ...
                  return NotificationListener<ScrollNotification>(
                    onNotification: (ScrollNotification scrollInfo) {
                      if (!scrollInfo.metrics.outOfRange &&
                        ...
                        context.read<BcoApplicationsBloc>().add(LoadMoreBcoApplications());
                      }
                      return false;
                    },
                    child: ListView.builder(
                      padding: const EdgeInsets.all(15),
                      itemCount: state.applications.length + (state.hasReachedMax ? 0 : 1),
                      itemBuilder: (context, index) {
                        ...
                        } else if (app.status.toUpperCase().contains('AWAITING')) {
  ...
  Widget _buildChip(BuildContext context, String label, bool active) {
    return GestureDetector(
      onTap: () {
        context.read<BcoApplicationsBloc>().add(FetchBcoApplications(status: label, isRefresh: true));
      },
      child: Container(
